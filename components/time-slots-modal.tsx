...
interface TimeSlotsModalProps {
  ...
  centerName: string
  date: string
  onFetchHours: (centerId: number, date: string, city: string) => Promise<AvailableHoursResponse[]>
}

export function TimeSlotsModal({ isOpen, onClose, centerId, centerName, date, onFetchHours }: TimeSlotsModalProps) {
  const [availableHours, setAvailableHours] = useState<AvailableHoursResponse[]>([])
